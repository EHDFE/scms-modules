...
define([
    ...
    return function(app, elem, attrs, scope) {
        app.directive('weekPickerDirective', ['G','$rootScope', function(G, $rootScope) {
            return {
                    require: '?ngModel',
                    scope: {
                        ngModel: '=',
                    ...
                    controller: ['$scope', '$element', '$attrs', function($scope, $element, $attrs) {
                        ...
                        $element.datetimepicker({
                            ...
                            $(this).data("DateTimePicker").widget.find('.datepicker-days .dow').css({'width':'12.49%'});
                        });
                        // $element.data("DateTimePicker").widget.find(".picker-switch a.btn").hide();
                        $element.data("DateTimePicker").widget.find(".picker-switch.accordion-toggle").replaceWith('<li class="clearWeek"><a class="btn" style="width:100%"><span>清除</span></a></li>');
                        $element.data("DateTimePicker").widget.find(".clearWeek").on('click',function(){
                        ...
                        function setWeek(num){
                            if($element.val()){
                                ...
                                if(num){
                                    var now = new Date();
                                    var weekDate = new Date(now.getFullYear(),now.getMonth(),now.getDate() - num * 7);
                            ...
                            setTimeout(function(){
                                ...
                                $element.data("DateTimePicker").widget.find('.day.active').parent().find('.day').addClass('active');

                                weekStyle();
                                $('.prev, .next, .month').off().on('click',function(){
                                    setTimeout(function(){
                                        weekStyle();
                                    },50);
                                });
                        ...
                        $scope.$watch('defaultWeek',function(newVal){
                            if(newVal||newVal===0){
                                    var now = new Date();
                                    var weekDate = new Date(now.getFullYear(),now.getMonth(),now.getDate() - newVal * 7);
                                    var y = weekDate.getFullYear();
                                    var m = weekDate.getMonth() + 1;  
                                    m = m < 10 ? '0' + m : m;  
                                    var d = weekDate.getDate();
